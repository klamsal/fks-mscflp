...
\begin{enumerate}
  \item \textbf{Kernel MILP.}
    ...
    where $\gamma = \text{mean}\{\rc{ji} : j \in \K,\; \rc{ji} > 0\}$ is the
    single global mean of the positive reduced costs over all kernel arcs.
...
satisfies $\rc{ji} \leq \gamma$, where $\gamma$ is a single global scalar ---
the mean of the positive reduced costs over all kernel arcs.
Two concerns follow.
