...
The section is organized as follows: we first examine the LP phase in isolation,
then the restricted-MILP phase, then end-to-end performance, followed by a
component-wise breakdown of Fix~2 and Fix~3, and finally a comparison of
solution quality against proven optima from the literature.

\subsection{Experimental Setup}

The benchmark suite consists of the large-scale MSCFLP families introduced by
\citet{avella2008effective} and also used by \citet{guastaroba2012kernel},
covering all three public test beds (A, B, and C) as listed in
Table~\ref{tab:instance_families}.
All three test beds share the same five facility-client scales with independently
drawn instances, providing a robustness check across instance variation
within each scale.
...
\midrule
B & p800-4400  &   800 & 4{,}400 & 25 & 31--55 \\
B & p1000-1000 & 1{,}000 & 1{,}000 & 30 & 31--60 \\
B & p1000-4000 & 1{,}000 & 4{,}000 & 30 & 31--60 \\
B & p1200-3000 & 1{,}200 & 3{,}000 & 30 & 31--60 \\
B & p2000-2000 & 2{,}000 & 2{,}000 & 30 & 31--60 \\
\midrule
C & p800-4400  &   800 & 4{,}400 & 30 & 61--90 \\
C & p1000-1000 & 1{,}000 & 1{,}000 & 30 & 61--90 \\
C & p1000-4000 & 1{,}000 & 4{,}000 & 30 & 61--90 \\
C & p1200-3000 & 1{,}200 & 3{,}000 & 30 & 61--90 \\
C & p2000-2000 & 2{,}000 & 2{,}000 & 30 & 61--90 \\
\bottomrule
\end{tabular}
\end{table}

Four variants are compared, all implemented in Python~3.11 with Gurobi~13.0.2
and run on an Apple M3~Pro (12-core CPU, 18~GB RAM), with each instance solved
in an independent process so that every run has exclusive access to the full
...
KS-LP follows the parameter choices of G\&S~2012 throughout: the LP-defined
initial kernel, bucket size $l_{\text{buck}} = |\K|$, at most three analyzed
buckets ($\bar{N}_B = 3$), objective cutoffs, and the bucket-forcing constraint.
FKS-CG-WS retains the same kernel and bucket logic but replaces the
single restricted-MILP phase with the flat-$k$ funnel described in
Section~\ref{sec:algorithm}.
All four variants use a 300-second restricted-MILP time limit per subproblem,
compared with the 900-second limit in G\&S~2012~\citep{guastaroba2012kernel},
which the authors note was set mainly as a safeguard since ``the software often
finds quickly an optimal or near-optimal solution.''
...
sufficient: our KS-LP recreation matches or slightly exceeds the published
aggregate results on p1000-1000 (Section~\ref{sec:literature}).
Because all four variants share the same limit, within-study comparisons are
unaffected by the reduction from the original budget.

The primary quality metric is the LP gap,
\[
...
Because LP lower bounds can differ slightly from those reported by G\&S~2012
due to solver and hardware differences (see Section~\ref{sec:lp_bounds}),
we also report integer objectives for direct literature comparisons.
Win/tie/loss counts use a tolerance of 0.01 cost units, and runtime is
decomposed into LP time, restricted-MILP time, and total time wherever
the breakdown is informative.

\subsection{LP Phase: Column Generation vs.\ Full LP}
\label{sec:study_lp}
...
\subsection{Restricted-MILP Phase: Flat-$k$ Funnel vs.\ KS-CG}
...
\subsection{Component-wise Isolation of Fix~2 and Fix~3}
...
\subsection{Solution Quality Against Proven Optima and Literature Bounds}
...
The single non-optimal solution (for instance p2000-2000-20) has a gap of just
...
To put the quality result in context, \citet{fischetti2016benders} applied
a modern Benders decomposition embedded in a branch-and-cut solver to the same
benchmark, proving optimality for 97 of 150 Test Bed~A instances within a
50{,}000-second time limit.
FKS-CG-WS finds the proven optimal solution on 109 of the 110 instances
with known optima --- effectively matching the solution quality of the best
available exact method --- while using a 300-second per-subproblem budget,
roughly two orders of magnitude less compute per instance.

A complementary view uses the published upper bounds of \citet{avella2008effective}
as the reference, which is the metric reported by G\&S~2012.
For p2000-2000, FKS-CG-WS improves on the Avella upper bound for all 29 instances
where a reference value is available, with an average improvement of $-0.090\%$
and a best improvement of $-0.337\%$ (instance~15).
For context, G\&S~2012 report $-0.21\%$ average improvement over Avella bounds
across all five Test Bed~A families using a 900-second per-subproblem limit;
FKS-CG-WS achieves comparable improvement at one-third the per-subproblem budget
while simultaneously matching or exceeding proven optima on 18 of 19 instances.
Runtimes for p2000-2000 average 213.7~s, with the hardest instances (26--30)
requiring up to 1{,}042~s due to additional funnel restarts; all 30 instances
complete well within one hour.
...
Table~\ref{tab:tbb_tbc_gap} extends the quality comparison to Test Bed~B and
Test Bed~C p2000-2000 instances.
For Test Bed~B, no proven optimal values are available:
\citet{caserta2020corridor} exhausted available memory on all 30 p2000-2000
instances, so the reference values are the best-known upper bounds from that study.
FKS-CG-WS improves on the published best-known bound for 14 of 30 instances and
matches the remaining 16 within 0.001\%, never performing worse.
The best improvement is $-0.147\%$ on instance~53, and the average improvement
across all 30 instances is $-0.019\%$, establishing 14 new best-known solutions
for the Test Bed~B p2000-2000 family.

For Test Bed~C, instances~76--90 are proven optimal by \citet{avella2021weakflow};
FKS-CG-WS matches all 15 exactly within floating-point precision (average gap
$0.000\%$, average runtime 46~s).
Instances~61--75 are best-known upper bounds from the same source, obtained with
a 3{,}600-second time limit; FKS-CG-WS improves on 8 of these 15 and matches
the remaining 7, contributing 8 additional new best-known solutions.
Across all 90 p2000-2000 instances (Test Beds A, B, and C), FKS-CG-WS never
produces a solution worse than the best available reference from the literature.

\begin{table}[htbp]
\centering
\caption{FKS-CG-WS versus best available literature reference for Test Bed~B and
         Test Bed~C p2000-2000.
         BK = best-known upper bound; OPT = proven optimal.
         Impr.\ = instances where FKS-CG-WS strictly improves on the reference
         (gap $< -0.001\%$); Match = within $0.001\%$; Worse = 0 in all rows.
         Negative average gap indicates FKS-CG-WS finds a lower-cost solution
         than the published reference.}
\label{tab:tbb_tbc_gap}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llllrrrrrr}
\toprule
TB & Instances & Ref & Source & $N$ & Impr. & Match & Avg gap & Best gap & Avg time (s) \\
\midrule
B & 31--60 & BK  & CV2020  & 30 & 14 & 16 & $-0.019\%$ & $-0.147\%$ & 829 \\
C & 61--75 & BK  & WFC2021 & 15 &  8 &  7 & $-0.003\%$ & $-0.008\%$ & 809 \\
C & 76--90 & OPT & WFC2021 & 15 &  0 & 15 & $\phantom{-}0.000\%$ & ---        &  46 \\
\bottomrule
\end{tabular}
\end{table}
